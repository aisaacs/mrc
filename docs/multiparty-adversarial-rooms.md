...
**The auth saga (2026-06-12 — file-delivery fixes; ⚠ SUPERSEDED 2026-06-16: the clone was retired entirely, so a summoned adversary no longer boots on *cloned* creds — see "cloned-token 401 → RESOLVED" below. Kept for the firewall fix, which still stands):**
...
- **cloned-token 401 → RESOLVED 2026-06-16 by RETIRING the clone (the whole "clone the login" arc above is superseded).** Root: cloning the summoner's volume copied its OAuth *refresh* token; the summoned Pierre's refresh ROTATED it (rotating refresh tokens are single-use), so the SUMMONER got logged out — reproduced live this session. A blank-refresh "migration" was specced, then dropped for the user's simpler insight: a summoned adversary is just a normal session, so **don't clone** — give each Pierre a DEDICATED per-repo slot pool (`mrc-config-<hash>-pierre-N`, lowest free slot), each slot persisting its OWN independent login (high-water-mark: log in once per slot, free + immortal after). Summoner-safety is now **structural** — `-pierre-N` is provably disjoint from regular `-K` volumes, so a Pierre's refresh can never touch your token. `cloneVolume` was removed. The same atomic slot-claim also dissolved the regular-session `getExistingCount()+1` TOCTOU + fail-open (A6): `claimLowestFree` (`src/docker.js`) derives in-use slots from running containers' real config-volume MOUNTS (migration-robust), takes the lowest free slot via an atomic `O_EXCL` create (two racers serialize at the kernel; fail-closed on a docker hiccup), and a `sawClaim` walk feeds the "others present?" newSession signal so two concurrent launches don't both `--continue` the shared `/workspace/.mrc` transcript. Claims are GC'd by **PID-liveness** (claim body = launcher PID + a trailing-`\n` completeness sentinel; reaped ONLY on affirmative `process.kill(pid,0)→ESRCH`; a 48h backstop reclaims PID-reuse leaks) so a slept laptop can't re-open a live slot. **Red-teamed across ~6 Pierre rounds** — the adversary feature found a fistful of real orphans (including one in already-committed code) and signed off each fix. Airtight-later (parked, cross-platform cost): stamp the PID's start-time → drop the wall-clock backstop. Commits: `85c76bf` (retire clone → slot pool), `97ce2be` (A6 race-free claim + getExistingCount removed), `796ffa1` (codex-twin skip), `a97dd0a` (sawClaim + PID-liveness).
- **session-name disambiguation — BUILT + VERIFIED (2026-06-12).** `list_peers` showed every same-repo session by bare repo name (the daemon `label` is set once at register, never updated), so a human couldn't pick the right peer. A red-team (Pierre) **killed "name on Max"** — the cheap host watcher (`mrc.js` `nameWatcher`) ALREADY names sessions live; the only gap was a missing wire. Shipped: `peerList` resolves the name **most-durable-first** — in-memory label → the on-disk `session-names` map (the watcher populates it; survives daemon restarts + sessions launched before the wire) → repo basename — and renders `repo · name-or-(fresh) · active/idle · web · [id]`; a `relabel` control action + watcher push keep the live label fresh; `--new <name>` now seeds the label at register (was applied only at exit). The durable **map-fallback** (not the wire) is the robust source. Verified live: worked-in `rp`/`RP-Diet` show their generated names, a `--new fix-x` session shows `fix-x` immediately, fresh shows `(fresh)`. Known edge: a still-running `--new` adversary shows `(fresh)` — it's named only at exit. Follow-up surfaced: a 2nd session in a repo hits a fresh config volume → re-login (only summoned adversaries clone today) — extend the `--entrypoint`-fixed clone to all multi-instance launches (no overwrite) to remove it.
- **zombie-room cleanup — BUILT + verified (2026-06-15), see `docs/rooms-end-deprecation.md`.** A *failed/disconnected* adversary summon left an open `adversary-*` room that blocked the next summon (hit 3× this session). Fixed in layers: (1) #4 connectivity-aware guards count only *connected* adversaries (a ghost no longer blocks) — committed; (2) a **failed-boot orphan reaper** (`room-daemon.js` tick) actively sweeps never-connected `adversary-*` dirs while **PRESERVING** any room that ever `[connected` (real transcripts — the user's choice (b)); (3) the **`hasOtherConnected` keystone** so a dormant room no longer brakes a live one (#5) or misroutes a bare reply (`activeRoomFor`); (4) a dashboard **`delete`** control for manual pruning. Verified live (swept `5cf7db8696`/`b7cd38b7e7`, kept the real transcripts). **#7 (human hard-stop) → PURE-C** (close-the-tab is the stop; rooms are your own sessions). Removing `mrc rooms end` *itself* (CLI + control action + dashboard ⏹ End button + the container-side agent framing) — **DONE 2026-06-17**, staged with the new in-band `invite_peer` verb (#13) for the image rebuild.
...
2. **✓ Tier 1 — `summon_adversary` (BUILT + VERIFIED LIVE 2026-06-11).** *Owner: the most appealing tier —
   ...
   skipped** for summoned sessions (`skipOp` — no naming-key Touch ID), and a **dedicated login pool**
   (a per-repo `mrc-config-<hash>-pierre-N` slot — NOT a clone of your login; each slot logs in once and is
   reused, so a Pierre can never orphan your session's token; superseded `cloneVolume`, which was removed —
   see the "cloned-token 401 → RESOLVED" entry above). Spawn is **hard-constrained** (fixed
