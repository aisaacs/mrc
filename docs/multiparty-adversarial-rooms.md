...
- **zombie-room cleanup — DECIDED + SPEC'd (2026-06-12), see `docs/rooms-end-deprecation.md`.** A *failed/disconnected* adversary summon left an open `adversary-*` room that blocked the next summon (hit 3× this session). #4 connectivity-aware guards (count only *connected* adversaries — a ghost no longer blocks) are DONE in `room-daemon.js` (pending `mrc rooms restart`). Decided: **remove `rooms end` entirely** + **grace-reap (~45s) adversary rooms on disconnect** + peer rooms kept forever + a dashboard delete. Phase 1 reaping/removal not yet built.
