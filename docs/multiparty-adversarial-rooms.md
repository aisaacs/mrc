...
- **container-setup fresh-volume login — file-delivery FIXED; token-validity OPEN (corrected 2026-06-15).** Root of the *missing*-creds failure: `cloneVolume` `cp -a`'d a *live* source volume, so the churny `claude.json` was skipped mid-write → container-setup restored a stale backup → login. Fix (`src/docker.js`, host-side, **UNCOMMITTED/held**): verify-retry both auth files until they land AND parse as JSON, re-seed a credential-less stale `-N` (+ the committed `--entrypoint sh`). This fixed FILE DELIVERY — a fresh `--new` boots in. **BUT** a later summon (2026-06-13) exposed the deeper root: the clone now delivers valid-JSON creds yet the OAuth token is **REJECTED (401)** — stale/rotated, because cloning shares one login across sessions. So clone-login is **NOT reliable end-to-end**; the **401 / token-rotation root is OPEN and undiagnosed** (credential-handling, human-gated). The `instanceId > 1` multi-instance clone-widening was **reverted 2026-06-15** (held pending the 401 root — a clean first-time login beats a stale-clone 401).
