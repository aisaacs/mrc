...
function connect() {
  ...
  sock.on('connect', () => {
    ...
    lastPong = 0
    try {
      sock.write(JSON.stringify({ type: 'register', sessionId: SESSION_ID, repo: REPO, label: LABEL, room: ROOM || undefined, notifyPort: NOTIFY || undefined, memberHandle: MEMBER || undefined, summonedBy: SUMMONED_BY || undefined, repoPath: REPO_PATH || undefined, adversary: process.env.MRC_ADVERSARY ? true : undefined, secret: process.env.MRC_ROOM_SECRET || undefined, ackEpoch: inbound.epoch() || undefined, ackSeq: inbound.highest() }) + '\n')   // summon/cage fields: daemon classifies from the HOST RECORD (not these), but repoPath enables summon-onto-same-repo and secret (#44) auths a reconnect. t27 resume token: ackEpoch/ackSeq report our dedup state so the daemon distinguishes a FLAP (epoch === its boot nonce → kept state, trim to ackSeq) from a FRESH process (epoch null → high-water 0 → resequence pending to 1..K). A fresh process has epoch=null → the daemon resequences; a flap carries the daemon's own epoch back.
