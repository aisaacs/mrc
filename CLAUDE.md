...
12. **`Dockerfile`** — Builds on `node:24-slim`. Installs Claude Code via native binary download, installs plugins from the official marketplace, installs Codex CLI (OpenAI), creates a non-root `coder` user, and grants passwordless sudo only for the firewall script. The container starts as **root** (so the entrypoint can apply the firewall), and `gosu` is installed for the privilege drop to `coder`. There is **no sudo** — the `sudo` package is removed and `coder`'s password is locked, so a sandboxed session has no escalation path (C/#38).

13. **`entrypoint.sh`** — Container startup, **root-init (C/#38)**: the container starts as root, so the entrypoint's _root pass_ waits for DNS (up to 30s) and applies the firewall **as root**, then drops to the unprivileged `coder` user via `gosu` and re-execs itself. The _coder pass_ asserts the firewall ran (`/etc/mrc-cage-profile` exists, else fatal), runs `container-setup.js` for config merging, optionally logs in Codex, then starts the selected agent (`claude` or `codex`). A sandboxed session can no longer invoke or weaken its own firewall.
...
- **`container/mrc-channel-server.js`** — container-side MCP "channel" server (shipped as the `room` plugin in a baked-in local marketplace `mrc-marketplace/`; loaded via `--channels plugin:room@mrc`, allowlisted in managed-settings so there's no prompt). Connects to the daemon and exposes `list_peers`/`ask_peer`/`reply`/`update_notes`/`pause_room`/`resume_room`/`submit_handoff`/`invite_peer`/`leave_room` (`invite_peer` pulls another live session into your CURRENT room → in-band N-party; `leave_room` drops just yourself from one room, history kept — the granular, non-destructive alternative to closing the tab) plus the **adversary** verbs — `summon_adversary` (a private red-team session, "Pierre"), `summon_adversary_to_room` (a fresh adversary into a shared room), and `accept_adversary`/`decline_adversary` (consent) — pushing peer messages into the session as `<channel>` tags. A summoned adversary launches under a hardened firewall profile (no `--web`, minimal allowlist, DNS-pinned) and gets its OWN dedicated login-slot pool (`mrc-config-<hash>-pierre-N`, lowest free slot) — **never a clone of your login**. Each Pierre slot logs in once and is reused (its own independent grant), so a summoned adversary can never rotate/orphan your session's token. (The earlier clone-the-login approach did exactly that — it copied your refresh token, and when Pierre refreshed it logged _you_ out; `cloneVolume` was removed.) The unused Codex twin volume is skipped for adversaries.
...
- **Multi-instance support** — Multiple `mrc` instances run against the same repo, each with its own config volume (`mrc-config-<hash>-2`, `-3`, …) and dynamically allocated proxy ports. The slot is claimed **race-free + fail-closed** (`claimLowestFree` in `src/docker.js`): in-use slots are derived from running containers' actual config-volume mounts, the slot is won by an atomic `O_EXCL` claim file carrying the launcher's PID, and the claim is GC'd by **PID-liveness** (reaped only when the launcher is _affirmatively_ dead — `process.kill(pid,0)→ESRCH` with a completeness sentinel; a 48h backstop reclaims PID-reuse leaks). So two concurrent launches can't grab the same volume, and a slept laptop can't re-open a live slot. A docker hiccup fails the launch closed (retryable) rather than colliding two sessions onto one `~/.claude` (which would share an OAuth refresh token → a silent logout).
...
Commands:
  ...
  mrc rooms brake|resume [room-id]        Pause / resume a room (no "close" — a room goes dormant when a session leaves)
