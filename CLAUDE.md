...
- **Codex image paste rides a fake `powershell.exe`** — the same inversion as the status line, and the reason `--agent codex` needs clipboard plumbing of its own. Claude Code *shells out* to `xclip`, which is the whole reason `clipboard-shim.sh` works: a command boundary is a place mrc can stand in. Codex links **arboard** and speaks X11 **in-process**, so the xclip shim is invisible to it and Ctrl+V dies at `arboard::Clipboard::new()` with `clipboard unavailable: … X11 server connection timed out` — before any image is even read. Codex has exactly **one** clipboard path that crosses a command boundary: its **WSL fallback**, which on that error runs `powershell.exe -NoProfile -Command <script>`, reads **stdout as a Windows path** to a PNG, and maps `C:\dir\f.png` → `/mnt/c/dir/f.png` before attaching the file. So mrc installs `codex-paste-shim.sh` as `/usr/local/bin/powershell.exe`, and `entrypoint.sh` exports `WSL_DISTRO_NAME` **for the Codex process only** — that env var alone flips Codex's `is_probably_wsl()`, and scoping it there keeps the rest of the container from thinking it is on WSL. The shim pulls `image/png` from the same clipboard proxy the xclip shim uses, so paste stays **on demand** — no X server, no Xvfb, no clipboard polling. Every drift mode here is silent (the shim runs, Codex still shows its original X11 error, nothing says mrc was involved), so `test/codex-paste-shim.test.mjs` pins the joins no single file can see: the printed path must be `C:\…` (**anything else is discarded**, empirically — a bare `/tmp/x.png` maps to `None`), it must map back to the directory the shim actually wrote, and the Dockerfile must pre-create `/mnt/c` (`coder` cannot `mkdir` in `/`). Diagnostics go to `/tmp/mrc-codex-paste-shim.log`, **never stdout**, which Codex takes verbatim as the path. `exit 1` means no image — which is also what a text-only clipboard yields, so Ctrl+V with text copied still shows the X11 error, and that is upstream's message to fix.
...
There is no build system or linter. The host-side logic has a **Node.js test suite** (`node --test test/*.test.mjs` — 355 tests covering the room engine + `@user` inbox, the daemon round-trips, Telegram transport/auth, the trust-marker defang, CSRF, org isolation, the worker runner, and the Codex config/paste-shim contracts); the container-launch and worker-exec paths still need a Docker rebuild to validate (see `docs/agent-teams.md`). Otherwise the project is Node.js modules (`mrc.js` + `src/`), a Dockerfile, shell scripts for the container entrypoint and firewall, and Node.js container scripts (`container/`).
