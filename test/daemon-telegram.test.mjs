...
test('daemon telegram (#24): the pushed question carries the SAME #N as the dashboard/CLI (cross-surface ref)', async () => {
  process.env.HOME = fs.mkdtempSync(`${os.tmpdir()}/mrc-tgiso-`)
  const port = await findFreePort(19460), controlPort = await findFreePort(port + 1)
  const tg = fakeTelegram()
  ...
  const push = tg.sent.find((m) => /ship it/.test(m.text))
  assert.ok(push, 'the question was pushed to Telegram')
