...
export function startRoomDaemon({ port, controlPort, notifyPort, dashboardPort = 0, turnCap = 200, triageWindowMs = (Number(process.env.MRC_TRIAGE_WINDOW) || 300) * 1000, stallMs = 600_000, version = '', idleMs = 600_000, tickMs = 15_000, dashboardKeepaliveMs = 30_000, catchupTimeoutMs = CATCHUP_TIMEOUT_MS, roomTtlMs = 300_000, workerInvoke = defaultWorkerInvoke, workerPollMs = 2_000, tgFetch = globalThis.fetch, tgToken, electSingleton = false, modelBPredict = () => { try { return decideModelB(imageIdAndLabels().labels) } catch { return false } } }) {
  ...
  function reapTransientSessions(org, handles) {
    for (const h of (handles || [])) {
      ...
      try { const r = loadSessionRecord(memberSessionId(org, h)); if (r && r.credsVol && r.claudeVol) { const err = syncCredentialVolume(r.claudeVol, r.credsVol); daemonLog(`[creds exit-sync] @${h}: ${r.claudeVol} → ${r.credsVol} = ${err ? 'FAILED: ' + err : 'OK'}`) } else { daemonLog(`[creds exit-sync] @${h}: SKIP (record has ${r && r.credsVol ? '' : 'no credsVol'}${r && r.claudeVol ? '' : ' no claudeVol'} — pre-d3fafa8 launch or non-caged)`) } } catch (e) { daemonLog(`[creds exit-sync] @${h}: THREW ${e?.message || e}`) }   // #66 diag: syncCredentialVolume returns '' on success or the error string on failure
    }
  }
      ...
      while ((i = buf.indexOf('\n')) >= 0) {
        ...
        if (f.action === 'launchteam') {
          ...
          try {
            ...
            const _web = !!_def?.web   // #57: per-project egress — def.web is the SINGLE source of truth (Pierre t95: no `?? roster.web` fallback, which the dashboard toggle reflecting def.web could never see → OFF-masquerade from the other end). One persisted writer (setorgweb), one launch source, one toggle. Threads --web into `mrc team up` → each live memberArgv (caged members excluded there).
        ...
        if (f.action === 'resumeconsult' && f.org && f.summonerHandle) {
          ...
          try {
            ...
            setTimeout(() => { teamMod.launchTransientConsult(f.org, built.repo, rosterPath, built.pierreMember, resumePrime).then((res) => { daemonLog(`resumeconsult ${f.org} launch result @${built.pierreHandle}: ${res?.ok ? 'OK (entry recorded)' : ('FAILED — ' + (res?.error || 'unknown'))}`) }).catch((e) => daemonLog(`resumeconsult ${f.org} launch THREW: ${e?.stack || e?.message || e}`)).finally(clearResume) }, 800)   // > killMember's 600ms SIGKILL escalation (matches relaunchmember's proven timing) so the respawn never races a still-dying master. ALWAYS-log the result (diag).
        ...
        if (f.action === 'addmember' && f.org) {
          ...
          try {
            ...
            const updated = teamMod.addMemberToRoster(teamMod.rosterFromDef(def), team, { role: f.role, backend: f.backend, territory: f.territory, name: f.name, repo: f.repo, lead: f.lead })   // #45/#46b: the added agent's OWN repo (Model B) + ★ lead — the dashboard authorized the repo first (human pick = grant)
